  ...
  async function deleteCustomer(eMail, apiContext) {
    ...
    if (data && data.length > 0) {
        ...
        for (const customer of data) {
            ...
            console.log(`Lösche Kunde mit ID: ${id}`);
            
            const deleteRes = await apiContext.delete(`/api/customer/${id}`);
            
            if (deleteRes.ok()) {
                console.log(`✅ Kunde ${id} erfolgreich gelöscht`);
            } else {
                console.log(`❌ Löschen von Kunde ${id} fehlgeschlagen: ${deleteRes.status()}`);
            
            }
        }
    } else {
        console.log('⚠️  Kein Kunde zum Löschen gefunden');
    }
}
