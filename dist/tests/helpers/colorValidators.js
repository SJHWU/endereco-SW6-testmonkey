...
export function isLightGreenish(colorString) {
  ...
  // - Grün sollte strikt höher sein als Rot und Blau
...
export function isLightYellowish(colorString) {
  ...
  return (
    ...
    b >= 238 && b <= 255 &&
    r >= g && g >= b &&
    (r - b) > 2 // Muss ein echter Warmton sein, schließt reines Weiß/Grau aus
  );
}
...
/**
 * Prüft ob eine CSS-Farbe im blauen Spektrum liegt
 * Zielwert: rgb(0, 66, 160) mit Toleranz
 * @param {string} colorString - CSS color string
 * @returns {boolean} True wenn die Farbe bläulich ist
 */
export function isBlueish(colorString) {
  const rgbMatch = colorString.match(/rgba?\((\d+),\s*(\d+),\s*(\d+)/);
  if (!rgbMatch) return false;
  
  const [, r, g, b] = rgbMatch.map(Number);
  
  // Toleranzbereich um rgb(0, 66, 160)
  // R: 0 ± 30, G: 66 ± 30, B: 160 ± 30
  const tolerance = 30;
  return (
    r >= 0 && r <= 30 &&
    g >= 36 && g <= 96 &&
    b >= 130 && b <= 190 &&
    b > r && b > g  // Blau muss dominant sein
  );
}